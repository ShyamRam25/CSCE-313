...
void traceroute(char* dest) {
    ...
    for (int ttl = 1; ttl <= MAX_HOPS; ) {
        ...
        while (1) {
             ...
             * 
             */
            if (sendto(sockfd, send_buf, sizeof(icmpheader), 0, (sockaddr*)&addr, sizeof(addr)) <= 0) {
            ...
            if (ret == 0) {
                ...
                int bytes = recvfrom(sockfd, recv_buf, sizeof(recv_buf), 0, (struct sockaddr*) &addr, &len);
                ...
                icmpheader* icmp2 = (icmpheader*)(icmp + sizeof(icmp));

                if (bytes >= 2 * (sizeof(ipheader) + sizeof(icmpheader))) {
                    ipheader * ip = (ipheader*)recv_buf;

                    icmpheader* icmp = (icmpheader*)(ip + 1);
                    ipheader* ip2 = (ipheader*)(icmp + 1);
                    icmpheader* icmp2 = (icmpheader*)(ip2 + 1);
                ...
                else if (bytes >= sizeof(icmpheader)) {
                    ipheader* ipref = (ipheader*)recv_buf;
                    icmpheader* icmpref = (icmpheader*)(ipref + 1);
